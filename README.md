...
Dự án được tổ chức theo nguyên tắc **Module hóa (Feature-based)**. Mỗi chức năng được tách biệt hoàn toàn ở cả 3 tầng.

```
AssignmentApp/
├── 📂 GUI (Presentation Layer)
│   ├── 📂 Base/                --> ucBase, frmBase (Ép chuẩn 1000x700, màu nền F0F2F5)
│   ├── 📂 Forms/               --> frmMain (Điều hướng), AuthForm (Đăng nhập)
│   └── 📂 UserControls/        --> Chia theo Role: Admin, Sales, Warehouse
├── 📂 BLL (Business Logic Layer)
│   ├── 📂 Services/
│   │   ├── 📂 Main/            --> AuthService.cs (Xử lý Đăng nhập), MainService.cs (Phân quyền)
...
Để hoàn thiện một chức năng (ví dụ: Quản lý Sản phẩm), thành viên thực hiện theo 4 bước:

1.  **Thiết kế UI**: Mở `AssignmentApp/GUI/UserControls/Warehouse/ucProductList.Designer.cs`. Kéo thả linh kiện Guna2 vào. Lưu ý: Mọi UC đều kế thừa `ucBase` nên đã có sẵn kích thước chuẩn.
2.  **Viết DAL**: Mở `AssignmentApp/DAL/Repositories/Warehouse/ProductRepository.cs`. Viết các câu lệnh SQL bằng Dapper.
    *   *Dấu hiệu*: `string sql = "SELECT * FROM tblHangHoa WHERE MaHang = @MaHang";`
3.  **Viết BLL**: Mở `AssignmentApp/BLL/Services/Warehouse/ProductService.cs`. Gọi hàm từ Repository và xử lý logic (nếu có).
4.  **Kết nối GUI**: Tại `AssignmentApp/GUI/UserControls/Warehouse/ucProductList.cs`, gọi hàm từ `ProductService` để hiển thị dữ liệu lên GridView.
...
| **Bảo mật BCrypt** | Mã hóa mật khẩu kèm Salt ngẫu nhiên, chống bẻ khóa. | Xem `AssignmentApp/BLL/Utils/PasswordHasher.cs`. Sử dụng thư viện `BCrypt.Net`. |
...
> 1. Kiểm tra kết nối SQL trong `AssignmentApp/DAL/Core/DbContext.cs` và cập nhật đúng Server của tôi.
> 2. Chạy tệp `AssignmentApp/DAL/Scripts/Database.sql` để tạo các bảng mẫu và tài khoản Test.
> 3. Chạy `dotnet restore` để nạp các thư viện: Guna2, Dapper, BCrypt, LiveChartsCore.
> 4. Hướng dẫn tôi cách sử dụng file `AssignmentApp/DAL/Repositories/Warehouse/ProductRepository.cs` làm mẫu để tôi tự làm module của mình."
